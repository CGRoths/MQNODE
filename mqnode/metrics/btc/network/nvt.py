...
def calculate_nvt(db, bucket_start_utc: datetime, interval: str) -> None:
    with db.cursor() as cur:
        if interval == '30m':
            cur.execute('SELECT * FROM btc_primitive_30m WHERE bucket_start_utc = %s', (bucket_start_utc,))
            row = cur.fetchone()
            if not row:
                return
            transferred_sat = row['transferred_sat_30m'] or 0
            ...
            if len(rows) == 0:
                return
            transferred_sat = sum(r['transferred_sat_30m'] or 0 for r in rows)
        ...
        cur.execute(
            ...
            INSERT INTO {table}(
              ...
              transferred_sat_30m, transferred_value_usd, nvt_raw,
