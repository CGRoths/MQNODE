...
from mqnode.queue.jobs import BTC_MARKET_QUEUE, BTC_MINER_QUEUE, BTC_NETWORK_QUEUE, BTC_PRIMITIVE_QUEUE, BTC_RAW_QUEUE
...
def enqueue_raw_block_ready(height: int) -> None:
    _queue(BTC_PRIMITIVE_QUEUE).enqueue('mqnode.workers.btc_primitive_worker.process_raw_block_job', {'chain': 'BTC', 'event': 'raw_block_ready', 'height': height})
...
def enqueue_metric_job(metric_name: str, interval: str, bucket_start_utc: datetime) -> None:
    ...
    payload = {'chain': 'BTC', 'event': 'metric_run', 'metric_name': metric_name, 'interval': interval, 'bucket_start_utc': bucket_start_utc.isoformat()}
