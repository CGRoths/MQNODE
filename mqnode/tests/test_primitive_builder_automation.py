...
def test_rebuild_10m_bucket_is_idempotent(monkeypatch):
    ...
    monkeypatch.setattr(primitive_builder, 'build_10m_bucket_payload', lambda cur, value, settings=None: {
        ...
        'high_price_usd': 88500,
        'low_price_usd': 87900,
        'close_price_usd': 88250,
        'market_cap_usd': 1000000,
        'onchain_volume_usd_raw_10m': 2,
...
def test_build_10m_bucket_keeps_price_fields_when_price_exists(monkeypatch):
    bucket = datetime(2026, 4, 20, 0, 0, tzinfo=timezone.utc)
    ...
    assert payload['close_price_usd'] == 88250
    assert round(payload['market_cap_usd'], 8) == round((100312500000 / 100_000_000) * 88250, 8)
