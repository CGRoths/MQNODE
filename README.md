...
- Missing price leaves price fields `NULL`; it never writes fake `0`.

The phase-0 canonical composer uses a robust median across available venue OHLC values and sums reported volumes. This is intentionally simple and replaceable later.
...
1. Raw venue fetchers normalize exchange OHLCV into raw `*_price_10m` tables.
2. The canonical composer reads all available venue rows for a bucket.
3. It writes `mq_btc_price_10m` with UPSERT.
4. The price composer checkpoint advances only after the write succeeds.
5. If no venue data exists for a bucket, the canonical price bucket can remain absent and primitive price fields stay `NULL`.
